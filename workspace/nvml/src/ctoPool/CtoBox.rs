...
pub struct CtoBox<T: CtoSafe>(*mut T, CtoPool);

// TODO: CtoRc
// TODO: CtoArc
// TODO: Puzzle: Do we always want to drop when there are no more references?
// TODO: There is a root object in every pool. The children should only be dropped when when they are no longer parented.
...
impl<T: CtoSafe + Clone> Clone for CtoBox<T>
{
	...
	fn clone(&self) -> Self
	{
		...
		unsafe { Self::from_raw(pointer, &self.1) }
	}
}
...
{
	/// Takes a value from C previously obtained using `into_raw()`.
	#[inline(always)]
	pub unsafe fn from_raw(raw: *mut T, cto_pool: &CtoPool) -> Self
	{
		CtoBox(raw, cto_pool.clone())
	}
