...
impl<'ctopool> CtoPoolAllocator<'ctopool>
{
	...
	#[inline(always)]
	pub fn allocate_root_box<T: CtoSafe + Send + Sync, InitializationError, Initializer: FnOnce(&mut T, &Self) -> Result<(), InitializationError>>(&self, initializer: Initializer) -> Result<CtoRootBox<T>, CtoPoolAllocationError<InitializationError>>
	{
